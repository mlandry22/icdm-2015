...
if __name__ == "__main__":
	...
	out_header = ['DV', 'cookie_id', 'device_id', 'prediction', 'drawbridge_handle', 'dummy_handle_minusone', 'mean_prediction_given_handle', 'min_prediction_given_handle', 'max_prediction_given_handle', 'count_given_handle', 'max_count_in_all_handle', 'max_prediction_all_handle', 'diff_from_max_prediction_all_handle']
	...
	for name, orig_group in merged_df_grouped:
		...
		max_pred_val = 0
		max_count_in_handle = 0
		for key in drawbridge_dict.keys():
				mean_val = np.mean(drawbridge_dict[key])
				min_val = np.min(drawbridge_dict[key])
				max_val = np.max(drawbridge_dict[key])
				mean_count_pred_dict[key] = [ mean_val, min_val, max_val, np.size(drawbridge_dict[key])]
				if mean_val > max_pred_val:
					max_pred_val = mean_val
				if np.size(drawbridge_dict[key]) > max_count_in_handle and key != "-1":
					max_count_in_handle = np.size(drawbridge_dict[key])

		for index,row in group.iterrows():
			handle = row['drawbridge_handle']
			out_row = [row['DV'], row['cookie_id'], row['device_id'], row['prediction'], row['drawbridge_handle'], row['dummy_handle_minusone'], mean_count_pred_dict[handle][0], mean_count_pred_dict[handle][1], mean_count_pred_dict[handle][2], mean_count_pred_dict[handle][3], max_count_in_handle, max_pred_val, max_pred_val-mean_count_pred_dict[handle][0]]
	...
	print "Processing test data..."
		...
		max_count_in_handle = 0
				...
				min_val = np.min(drawbridge_dict[key])
                                ...
                                if mean_val > max_pred_val:
                                        max_pred_val = mean_val
				if np.size(drawbridge_dict[key]) > max_count_in_handle:
                                        max_count_in_handle = np.size(drawbridge_dict[key])
